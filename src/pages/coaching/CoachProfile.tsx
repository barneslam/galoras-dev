...
export default function CoachProfile() {
  ...
  const proofPoints = useMemo(() => normalizeProofPoints(coach?.proof_points), [coach?.proof_points]);
  ...
  hover:text-foreground transition-colors"
            ...
            {loading && <div className="py-16 text-center text-muted-foreground">Loading profile...</div>}

            {!loading && debugError && <pre className="whitespace-pre-wrap text-red-500 text-sm">{debugError}</pre>}

            {!loading && !debugError && !coach && (
              <div className="py-16 text-center">
                <h1 className="text-2xl font-semibold mb-2">Coach not found</h1>
                <p className="text-muted-foreground">This profile is not currently available.</p>
              </div>
            )}
                ...
                <div className="rounded-2xl border border-border bg-card p-8 md:p-10 mb-8">
                  <div className="max-w-3xl">
                    <div className="flex flex-wrap items-center gap-3 mb-5">
                      <div
                        className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10
                    ...
                    {proofPoints.length > 0 ? (
                      <div className="space-y-4">
                        {proofPoints.map((point, index) => (
                          <div key={index} className="rounded-xl border border-border bg-muted/30 p-5">
                    ...
                    {products.length > 0 ? (
                      <div className="space-y-4">
                        {products.map((product) => (
                          <div key={product.id} className="rounded-xl border border-primary/20 bg-primary/5 p-6">
                            <div
                              className="flex flex-col gap-4 md:flex-row md:items-start
  md:justify-between"
                            >
                              <div className="max-w-2xl">
                                <h3 className="text-lg font-semibold mb-2">{product.title || "Untitled Product"}</h3>
      ...
      {checkoutOpen && selectedProduct && <CheckoutModal open={checkoutOpen} onOpenChange={setCheckoutOpen} />}

      {paymentError && !checkoutOpen && (
        <div
          className="fixed bottom-4 right-4 max-w-sm rounded-lg bg-destructive/10 border
  border-destructive/20 px-4 py-3 text-sm text-destructive shadow-md"
        >
          {paymentError}
        </div>
      )}
